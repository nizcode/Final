...
word_df<-unlist(tokenize_ngrams(dfc$Plot, n=2, n_min=2,stopwords = stopwords::stopwords("en")))#,strip_numeric = TRUE)
...
```
This analysis performed here using the ngram function proves exactly its usefulness. This is because imagine you solely analysed 1 word tokens, you may find a high number of "new" tokens and "york" possibly as well. These tokens went taken independently are meaningless, it is only when you put them together that you can form some sort of conclusion, ie. a lot of romance movie happen in New York.


Next I will be looking another very useful function that is part of the package, it is called tokenize_word_stems(x, language = "english", stopwords = NULL,
simplify = FALSE).

The output of this function is a character vector of word stems. For example, "love", "lovely", "loved" all belongs to the character stem "love".

This function is wrapper around the wordStem function from the R package called Snowball C. Snownball C package is an interface for the C 'libstemmer' librsry which implements a Stemming Algorithm written by Martin Porter in the Snowball language. Snowball is a small string processing language for creating stemming algorithms. A Snowball compiler exists to translates snowball programs into C. 


```{r}
library(ggplot2)

genre <- c("romance","crime","horror","sci")
plots <- list()
g = "drama"

dfc <- df0[grepl(g,df0$Genre,fixed=TRUE),]

word<-unlist(tokenize_word_stems(dfc$Plot, stopwords = stopwords::stopwords("en")))#,strip_numeric = TRUE)
word_df<-data.frame(sort(table(word),decreasing=TRUE))

word2<-unlist(tokenize_words(dfc$Plot, stopwords = stopwords::stopwords("en")))#,strip_numeric = TRUE)
word_df2<-data.frame(sort(table(word2),decreasing=TRUE))

names(word_df)[1] <- "word"
names(word_df2)[1] <- "word2"

  p <- ggplot(word_df[30:60,],
...
g <- ggplot(word_df2[30:60,],
     aes(x=word2, y=Freq,fill=word2))+
geom_bar(stat="identity") +
guides(fill = F)+
xlab(NULL)+
coord_flip()+
...
```
In order to portray the purpose of the stem function, a comparison of word frequency on horror plots between  the stem method and the non stem method. 

The stem "kill" which encompass the terms, killed, kills and killer comes more than twice as often in comparison that sole word "killed". Moreover it is the most frequently used token in the non-stem analysis, whereas "killed" is the tenth most frequent term. 

Moreover the term find in the stem analysis carries more of an importance when combining "find" and "finds" in comparison to looking at those terms independently.

In conclusion, we can theorize using the stem function and solely looking at the top 2 most frequent word, that a big proportion of horror movies has a house and murder crime, possibly most murderers happen in a house, in horror movies. 
  
  
```{r}
#100 most frequent words
#dfc <- df0[grepl("romance",df0$Genre,fixed=TRUE),]
desc_df<-tokenize_word_stems(df0$Plot, stopwords = stopwords::stopwords("en"))
descT<-tokenize_word_stems(df0[1:50,"Plot"], stopwords = stopwords::stopwords("en"))

g = "crime"
dfc <- df0[grepl(g,df0$Genre,fixed=TRUE),]
desc_df<-tokenize_word_stems(dfc$Plot, stopwords = stopwords::stopwords("en"))
word_df<-data.frame(sort(table(unlist(desc_df)),decreasing=TRUE))
word_df <- word_df[1:100,]

#i = 3
ln = length(desc_df[[3]])
cntw <- function(ln){
  cnts <- NULL
  for (i in 1:dim(word_df)[1]){
    cnts <- c(cnts,length(which(as.character(word_df[i,1])==ln)))
  }
  return(cnts)
}

c_m <- sapply(desc_df, cntw)
c_m <- t(c_m)

c_df <- data.frame(grepl(g,df0$Genre,fixed=TRUE),c_m)
names(c_df) <- c(g,as.character(word_df[,1]))
```
